...
const ModalProfile = (props) => {
    ...
    const handleImageChange = (event) => {
        ...
        if (file && /(\.jpg|\.jpeg|\.png)$/i.test(file.name)) {
            ...
            toast.error("Please select a file with a valid extension (JPEG, JPG, PNG).");
        }
    };

    const handleDownload = async (fileName) => {
        const response = await axiosInterceptors.get(REQ.DOWNLOAD_RESUME.replace(":id", candidateId));
    ...
    const handleSubmit = async (values, actions) => {
        ...
        try {
            if (candidateRegistered) {
                const response = await axiosInterceptors.put(REQ?.CREATE_CANDIDATE, formData);
                const userDetails = JSON.parse(gContext?.user);

                const updatedUserDetails = {
                    ...userDetails, can_code: response?.can_code
                };
                                        ...
                                        <Form>
                                            <div className="d-flex justify-content-center align-items-center">
                                                <ProfilePictureWrapper>
                                                    <ProfilePicture image={profileImage && URL.createObjectURL(profileImage)}>
